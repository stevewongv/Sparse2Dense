...
def load_state_dict(module, state_dict, strict=False, logger=None):
    ...
    for name,p in own_state.items():
        if name[4:] in state_dict:
            if isinstance(state_dict[name[4:]], torch.nn.Parameter):
                # backwards compatibility for serialized parameters
                param = state_dict[name[4:]].data
            else:
                param = state_dict[name[4:]]
            if param.size() != p.size():
                # shape_mismatch_pairs.append([name, own_state[name].size(), param.size()])
                continue
            own_state[name].copy_(param)
        elif name.count('single_det.') and name.replace('single_det.','') in state_dict:
            name_ = name.replace('single_det.','')
            if isinstance(state_dict[name_], torch.nn.Parameter):
                # backwards compatibility for serialized parameters
                param = state_dict[name_].data
            else:
                param = state_dict[name_]
            if param.size() != p.size():
                # shape_mismatch_pairs.append([name, own_state[name].size(), param.size()])
                continue
            own_state[name].copy_(param)
