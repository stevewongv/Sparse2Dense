...
S_model = dict(
    ...
    backbone=dict(type="PointPillarsScatter_S2D", ds_factor=1),
    neck=dict(
        type="RPN",
        layer_nums=[3, 5, 5],
        ds_layer_strides=[1, 2, 2],
...
voxel_generator = dict(
    ...
    distillation = True # we only use non-empty voxels. this will be much smaller than max_voxel_num
)

train_pipeline = [
    dict(type="LoadPointCloudFromFile", dataset=dataset_type, distillation = True),
...
load_from = './work_dirs/waymo_centerpoint_pp_two_pfn_stride1_3x_dense_interval_5/latest.pth'
resume_from = None
workflow = [('train', 1)]
distillation = True
